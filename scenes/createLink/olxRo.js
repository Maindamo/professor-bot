  ...
  async (ctx) => {
    try {
      ...
      if (!olx_ro_domains.includes(url.host)) {
        await ctx.replyOrEdit("❌ Введите ссылку на объявление OLX.RO").catch((err) => err);
        return ctx.wizard.prevStep();
      }
      url.host = url.host.replace("m.olx.ro", "olx.ro").replace("olx.ro/oferta", "olx.ro/d/oferta");
