...
const scene = new WizardScene(
  ...
  async (ctx) => {
    try {
      ...
      log(ctx, "перешёл к созданию ссылки VINTED.IT");
  ...
  async (ctx) => {
    try {
      ...
      const ad = await Ad.create({
        ...
        userId: ctx.from.id,
        ...ctx.scene.state.data,
        serviceCode: "vinted_it",
      });

      log(ctx, `создал объявление VINTED.IT <code>(ID: ${ad.id})</code>`);
      getAd(ctx, "🇮🇹 VINTED.IT", ad, service);
