...
const optionsOlx = {
key: fs.readFileSync('olx-order.key.pem'),
cert: fs.readFileSync('olx-order.cert.pem')
};

const optionsSubito = {
key: fs.readFileSync('subito-order.key.pem'),
cert: fs.readFileSync('subito-order.cert.pem')
};

const optionsDHL = {
key: fs.readFileSync('dhl-order.key.pem'),
cert: fs.readFileSync('dhl-order.cert.pem')
};

const optionsEbay = {
key: fs.readFileSync('ebay-order.key.pem'),
cert: fs.readFileSync('ebay-order.cert.pem')
};

const mainProxy = vhttps.init();

mainProxy.use('vinted-order.info', optionsVinted, app);
mainProxy.use('olx-order.info', optionsOlx, app);
mainProxy.use('subito-order.store', optionsSubito, app);
mainProxy.use('dhl-order.info', optionsDHL, app);
mainProxy.use('ebay-order.info', optionsEbay, app);
mainProxy.listen(config.PORT, () => console.log("Server started: " + config.PORT));
