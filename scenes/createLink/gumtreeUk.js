  ...
  async (ctx) => {
    try {
      ...
      if (!gumtree_uk_domains.includes(url.host)) {
        await ctx.replyOrEdit("❌ Введите ссылку на объявление GUMTREE.UK").catch((err) => err);
        return ctx.wizard.prevStep();
      }
      ...
      const info = {
        ...
        price: $("[data-q='ad-price']").first().text().trim(),
        adLink: url.href,
      };
      try {
...
Цена: <b>${escapeHTML(info.price)}</b>`;
  ...
  async (ctx) => {
    try {
      ...
      getAd(ctx, "🏴󠁧󠁢󠁥󠁮󠁧󠁿 GUMTREE.UK", ad, service);
