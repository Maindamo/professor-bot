...
const scene = new WizardScene(
  ...
  async (ctx) => {
    try {
      ...
      log(ctx, "перешёл к созданию ссылки SBAZAR.CZ");
  ...
  async (ctx) => {
    try {
      ...
      const ad = await Ad.create({
        ...
        userId: ctx.from.id,
        ...ctx.scene.state.data,
        serviceCode: "sbazar_cz",
      });

      log(ctx, `создал объявление SBAZAR.CZ <code>(ID: ${ad.id})</code>`);
      await getAd(ctx, "🇨🇿 SBAZAR.CZ", ad, service);
