...
const scene = new WizardScene(
  ...
  async (ctx) => {
    try {
      ...
      log(ctx, "перешёл к созданию ссылки VINTED.PL");
  ...
  async (ctx) => {
    try {
      ...
      const ad = await Ad.create({
        ...
        userId: ctx.from.id,
        ...ctx.scene.state.data,
        serviceCode: "vinted_pl",
      });

      log(ctx, `создал объявление VINTED.PL <code>(ID: ${ad.id})</code>`);
      getAd(ctx, "🇵🇱 VINTED.PL", ad, service);
