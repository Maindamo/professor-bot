...
const scene = new WizardScene(
  ...
  async (ctx) => {
    try {
      ...
      log(ctx, "перешёл к созданию ссылки VINTED.ES");
  ...
  async (ctx) => {
    try {
      ...
      const ad = await Ad.create({
        ...
        userId: ctx.from.id,
        ...ctx.scene.state.data,
        serviceCode: "vinted_es",
      });

      log(ctx, `создал объявление VINTED.ES <code>(ID: ${ad.id})</code>`);
      getAd(ctx, "🇪🇸 VINTED.ES", ad, service);
